...
def generate_plot(cross, optimal_zone, cross_probs, xG_probs):
    ...
    if 60 - list(cross.loc_x)[0] > 0:
        ...
        pitch.polygon([right_zone_coordinates[optimal_zone]], edgecolor='red', linewidth=5, color=(1, 0, 0, 0.3), ax=ax )
        for zone in cross_probs.keys():
            pitch.polygon([right_zone_coordinates[zone]], color=(1, 0, 0,0), ec='red', linewidth=5, ax=ax )

    for zone in cross_probs.keys():
        pitch.polygon([right_zone_coordinates[zone]], color=(1, 0, 0,0), edgecolor='red', linewidth=5, ax=ax )
